...
def main():
    ...
    example_inputs = (
        torch.randint(
            0, model_config.vocab_size, (1, model_config.sequence_len), dtype=torch.long
        ),
    )
    dynamic_shapes = {
        "idx": {1: torch.export.Dim("token_dim", min=1, max=model_config.sequence_len)}
    }
    with torch.inference_mode(), sdpa_kernel([SDPBackend.MATH]):
        ep_model = torch.export.export(
            model, example_inputs, dynamic_shapes=dynamic_shapes
        )

    print(f"Exporting to {args.pte_path}...")
    edge_config = EdgeCompileConfig(_check_ir_validity=False)
    edge_manager = to_edge(ep_model, compile_config=edge_config)
    et_program = edge_manager.to_executorch()

    with open(args.pte_path, "wb") as file:
